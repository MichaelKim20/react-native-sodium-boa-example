...
export default class Test extends BasicTest
{
    ...
    async _test() {
        ...
        Promise.all(sample_crypto_core_ed25519_random.map(t => {
            return sodium.crypto_core_ed25519_from_uniform(this.hex2bin(t.random))
                .then(q => t.crypto_core_ed25519_from_uniform === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
        ...
        Promise.all(sample_crypto_core_ed25519_add_sub.map(t => {
            return sodium.crypto_core_ed25519_add(this.hex2bin(t.p), this.hex2bin(t.q))
                .then(q => t.add === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
        ...
        Promise.all(sample_crypto_core_ed25519_add_sub.map(t => {
            return sodium.crypto_core_ed25519_sub(this.hex2bin(t.p), this.hex2bin(t.q))
                .then(q => t.sub === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_sub', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_sub', error))

        Promise.all(sample_crypto_core_ed25519_scalar_reduce.map(t => {
            return sodium.crypto_core_ed25519_scalar_reduce(this.hex2bin(t.hash))
                .then(q => t.result === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_reduce', results.find(t => t === false) === undefined))
        ...
        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_add(this.hex2bin(t.x), this.hex2bin(t.y))
                .then(q => t.add === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_add', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_scalar_add', error))

        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_sub(this.hex2bin(t.x), this.hex2bin(t.y))
                .then(q => t.sub === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_sub', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_scalar_sub', error))

        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_mul(this.hex2bin(t.x), this.hex2bin(t.y))
                .then(q => t.mul === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_mul', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_scalar_mul', error))


        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_negate(this.hex2bin(t.x))
                .then(q => t.negate_x === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_negate', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_scalar_negate', error))

        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_invert(this.hex2bin(t.x))
                .then(q => t.invert_x === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_invert', results.find(t => t === false) === undefined))
            .catch((error) => this.testFailed('crypto_core_ed25519_scalar_invert', error))

        Promise.all(sample_crypto_core_ed25519_scalar_xxxxx.map(t => {
            return sodium.crypto_core_ed25519_scalar_complement(this.hex2bin(t.x))
                .then(q => t.complement_x === this.bin2hex(q))
                .catch(e => {
                    throw e
                })
        }))
            .then(results => this.testPassed('crypto_core_ed25519_scalar_complement', results.find(t => t === false) === undefined))
